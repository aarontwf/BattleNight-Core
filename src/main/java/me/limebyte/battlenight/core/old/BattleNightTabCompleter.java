...
public class BattleNightTabCompleter implements TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) throws IllegalArgumentException {
        ...
        if (args.length == 1) return StringUtil.copyPartialMatches(args[0], CommandManager.getMainChoices(), new ArrayList<String>());

        if (args.length == 2) {
            for (BattleNightCommand command : CommandManager.getCommands()) {
                if (command.labelMatches(args[0]) || command.aliasMatches(args[0])) return StringUtil.copyPartialMatches(args[1], command.getPrimaryChoices(), new ArrayList<String>());
            }
        }
